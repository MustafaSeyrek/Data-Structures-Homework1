...
public abstract class AbstractHashMap<K, V, C> extends AbstractMap<K, V, C> {
	...
	@Override
	public V get(K key, V value, C count) {
		return bucketGet(hashValue(key), key, value, count);
	}

	@Override
	public V remove(K key, V value, C count) {

		return bucketRemove(hashValue(key), key, value, count);
	}

	@Override
	public V put(K key, V value, C count) {
		// System.out.println(key);
		V answer = bucketPut(hashValue(key), key, value, count);
		// System.out.println(key);
		/*
		 * if (n > capacity / 2) // keep load factor <= 0.5 resize(2 * capacity - 1); //
	...
	protected abstract V bucketGet(int h, K k, V v, C c);

	protected abstract V bucketPut(int h, K k, V v, C c);

	protected abstract V bucketRemove(int h, K k, V v, C c);
}